    ...
    <div class="card-body">
        <form action="{{ route('manutencoes.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select class="form-control" id="equipamento_id" name="equipamento_id" required>
                    ...
                    @foreach ($equipamentos as $equipamento)
                        <option value="{{ $equipamento->id }}" {{ old('equipamento_id', request('equipamento_id')) == $equipamento->id ? 'selected' : '' }}>
                            {{ $equipamento->nome }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="date" class="form-control" id="data_manutencao" name="data_manutencao" value="{{ old('data_manutencao') }}" required>
            </div>
